...
class GenerativeResnet(GraspModel):
    def __init__(self, input_channels=4, output_channels=1, channel_size=32, dropout=False, prob=0.0):
        ...
        self.vmamba = Backbone_VMAMBA2(
            in_chans=input_channels, patch_size=2, d_state=256, linear_attn_duality=True, embed_dim=embed_dim
        ).cuda()

        # Decoder (upsampling layers)
        self.up1 = nn.ConvTranspose2d(embed_dim * 8, embed_dim * 4, kernel_size=4, stride=2, padding=1)
        self.up2 = nn.ConvTranspose2d(embed_dim * 8, embed_dim * 2, kernel_size=4, stride=2, padding=1)  # Concatenate with encoder layer
        self.up3 = nn.ConvTranspose2d(embed_dim * 4, embed_dim, kernel_size=4, stride=2, padding=1)
        self.up4 = nn.ConvTranspose2d(embed_dim * 2, embed_dim * 2, kernel_size=4, stride=2, padding=1)
        self.up5 = nn.ConvTranspose2d(embed_dim * 2, channel_size, kernel_size=4, stride=2, padding=1)
