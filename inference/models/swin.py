...
class SwinTransformerBlock(GraspModel):
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=300, patch_size=4, input_channels=1, embed_dim=96, norm_layer=None):
        super().__init__()
    ...
    def flops(self):
        ...
        flops = Ho * Wo * self.embed_dim * self.input_channels * (self.patch_size[0] * self.patch_size[1])
        if self.norm is not None:
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, input_channels=1, num_classes=1,
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, input_channels=input_channels, embed_dim=embed_dim,
...
if __name__ == '__main__':
    ...
    # model = SwinTransformerSys(input_channels=1)
    # summary(model, (1, 224, 224))
    # model = SwinTransformerSys(input_channels=3).cuda()
    #
    #
    import time
    ...
    model = SwinTransformerSys(input_channels=1,embed_dim=24,num_heads=[3, 6, 12, 24]).cuda()
    # model = SwinTransformerSys(input_channels=1, embed_dim=12, num_heads=[1, 2, 2, 4]).cuda()
    # flops, params = profile(model, inputs=(input, ))
    # macs, params = get_model_complexity_info(model, (3, 224, 224), as_strings=True,
    #                                          print_per_layer_stat=True, verbose=True)
    # print('{:<30}  {:<8}'.format('Computational complexity: ', macs))
    # print('{:<30}  {:<8}'.format('Number of parameters: ', params))
    # model = SwinTransformerSys(input_channels=3, embed_dim=24, num_heads=[1, 2, 4, 8]).cuda()
    # stat(model, (3, 224, 224))
    summary(model, (1, 224, 224))
    # model = SwinTransformerSys(input_channels=1, )
    # model = SwinTransformerSys(input_channels=1, embed_dim=96, num_heads=[3, 6, 12, 24])
    # model = SwinTransformerSys(input_channels=1, embed_dim=96, num_heads=[1, 2, 4, 8])
    # model = SwinTransformerSys(input_channels=1, embed_dim=48, num_heads=[1, 2, 4, 8])
    model = SwinTransformerSys(input_channels=1, embed_dim=48, num_heads=[1, 2, 4, 8])
    # summary(model, (1, 224, 224))
    model.eval()
    sum=0.
